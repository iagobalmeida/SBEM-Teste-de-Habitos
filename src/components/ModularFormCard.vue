<template>
  ...
  >
    <form v-on:submit.prevent="$emit('nextStep')">
      <!-- Header -->
      <div class="card-header p-6" v-html="customHeader" v-if="customHeader"></div>
          ...
          <div :class="`form-group w-100 ${inputIndex == inputs.length - 1 ? 'mb-0' : ''}`" v-if="['text', 'email', 'tel', 'password'].includes(input.type)">
            <label>
                <b class="d-flex justify-content-center align-items-center">
                    <i class="fas fa-asterisk text-muted fa-xs"  v-if="input.required">&nbsp;</i>
                    {{ input.label }}:
                </b>
            </label>
            <input
      ...
      <!-- Footer -->
      <div class="card-footer" v-html="customFooter" v-if="customFooter"></div>
      <div class="card-footer" v-else>
        ...
        <button
          ...
          "
          tabIndex="0"
          v-bind:disabled="enableNextStep"
        >
          Avançar
  ...
  methods: {
    ...
    checkInput(input) {
      return typeof input.value === "object"
        ? this.checkComplexInput(input)
        : this.checkSimpleInput(input);
    },
    ...
    checkComplexInput(input) {
      switch (input.type) {
        case "checkbox":
          return Object.values(input.value).reduce(
            (acc, value) => acc || value
          );
  ...
  computed: {
    enableNextStep() {
      return !this.inputs.reduce(
        (acc, input) => (input.required ? this.checkInput(input) && acc : acc),
        true
      );
    }
  },
